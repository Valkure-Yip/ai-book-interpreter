...
Langfuse 默认**仅上传 metadata + token usage**，不上传原文与译文。
- 默认行为：trace.input/output 字段被 `providers/observability/langfuse.py` 置空
- 用户显式 `observability.langfuse.upload_full_payload: true` 或 `LANGFUSE_FULL_PAYLOAD=1` 才上传全文
- 自托管 Langfuse 实例可绕开此限制（数据不出网络边界），但仍受同一开关控制以保持一致行为
- lint `tools/lint/langfuse_payload.py` 守护未授权的全文上传路径
